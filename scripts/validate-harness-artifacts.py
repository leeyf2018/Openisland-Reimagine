...
def main() -> None:
    ...
    elif scenario == "grokCompletionCard":
        if notch_status != "opened":
            fail(f"expected opened notch for grokCompletionCard, got {notch_status!r}")
        if not (island_surface.startswith("completionCard:") or is_actionable_session_surface(island_surface)):
            fail(f"expected grokCompletionCard to remain on completion/actionable session surface, got {island_surface!r}")
        require_frame_between(
            overlay_frame,
            width=(640, 700),
            height=(260, 520),
            context="grokCompletionCard overlay frame",
        )
        assert_contains_any(text_values, ["多 Grok 孤儿会话"], "grokCompletionCard first digest item")
        assert_contains_any(text_values, ["摘要可视区域适度增高"], "grokCompletionCard fifth digest item")
        assert_contains_any(text_values, ["正式 App 和 Codex"], "grokCompletionCard seventh digest item")
