...
extension AgentSession {
    ...
    var spotlightActivityLineText: String? {
        ...
        case .completed:
            // Prefer distilled 3-bullet outcome digest — never echo the user prompt.
            if let digest = completionDigestText {
                return digest
            }
            if let assistantMessage = lastAssistantMessageText?.trimmedForSurface,
               !assistantMessage.isEmpty {
                return GrokActiveSessionDiscovery.buildCompletionDigest(from: assistantMessage, limit: 3)
    ...
    /// Distills the completion reply into **exactly up to 3** short bullets.
    var completionDigestText: String? {
        if let prebuilt = grokMetadata?.completionDigest?.trimmedForSurface,
           !prebuilt.isEmpty {
            return prebuilt
        }

        let source: String?
        if let assistant = lastAssistantMessageText?.trimmedForSurface, !assistant.isEmpty {
            source = assistant
        } else {
            let fallback = summary.trimmedForSurface
            // Reject summaries that are still the user prompt (Grok L1 bug).
            if fallback.isEmpty
                || fallback == (latestUserPromptText?.trimmedForSurface ?? "")
                || fallback.hasPrefix("You:")
                || fallback.hasPrefix("1. You")
                || (latestUserPromptText.map { fallback.contains($0.prefix(40)) } ?? false) {
                source = nil
            } else if fallback.contains("\n1.") || fallback.hasPrefix("1. ") {
                // Already a digest stored in summary.
                return fallback
            } else {
                source = fallback
            }
        ...
        return GrokActiveSessionDiscovery.buildCompletionDigest(from: source, limit: 3)
    }
