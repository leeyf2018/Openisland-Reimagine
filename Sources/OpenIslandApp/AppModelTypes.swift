...
enum IslandAnimatedStatusVisual: Equatable {
    ...
    ) -> IslandAnimatedStatusVisual {
        switch phase {
        case .running:
            return .runningWaveform
        case .waitingForApproval:
            return .approvalAttention
        case .waitingForAnswer:
            return .answerAttention
        case .completed:
            return presence == .inactive ? .idleDot : .completedBadge
        }
    }
}

enum IslandRunningWaveformMetrics {
    static let barCount = 3
    static let minimumBarHeight: CGFloat = 3
    static let maximumBarHeight: CGFloat = 11
    static let barWidth: CGFloat = 2
    static let barSpacing: CGFloat = 1

    static func barHeight(index: Int, phase: TimeInterval) -> CGFloat {
        let phaseOffset = Double(index) * ((2 * Double.pi) / Double(barCount))
        let normalized = (sin(phase + phaseOffset) + 1) / 2
        return minimumBarHeight + CGFloat(normalized) * (maximumBarHeight - minimumBarHeight)
    }
}
