...
final class AppModel {
    ...
    init(
        ...
        if UserDefaults.standard.object(forKey: Self.showCodexUsageDefaultsKey) != nil {
            ...
            showCodexUsage = FileManager.default.fileExists(atPath: CodexRolloutDiscovery.defaultRootURL.path)
                || FileManager.default.fileExists(atPath: GrokUsageLoader.defaultLogURL.path)
        }
        ...
        // Grok L1 has no hooks — turn/session completion is synthesized in
        // process monitoring and must use the same notification+sound path.
        monitoring.onAgentEvent = { [weak self] event in
            self?.applyTrackedEvent(
                event,
                updateLastActionMessage: false,
                ingress: .rollout
        ...
        // New Grok sessions only appear via polling. Force the island open so
        // the user actually sees the row (Codex hooks push immediately; Grok
        // previously waited on idle 300s / active 60s full reconcile).
        monitoring.onGrokSessionsAppeared = { [weak self] sessionIDs in
            guard let self, let sessionID = sessionIDs.first else { return }
            self.selectedSessionID = sessionID
            self.surfaceGrokSessionVisibility(sessionID: sessionID, reason: "new session")
        }
        monitoring.onGrokTurnStarted = { [weak self] sessionID in
            guard let self else { return }
            self.selectedSessionID = sessionID
            self.surfaceGrokSessionVisibility(sessionID: sessionID, reason: "turn started")
        }
    ...
    func applyTrackedEvent(
        ...
        // Guard: don't let an older rollout event downgrade a session from
        // completed back to running. A same-time or newer event can be the
        // first signal for a resumed turn when the app-server misses
        // turn/started, so it must be allowed to reopen the session.
        if ingress == .rollout,
           case let .activityUpdated(payload) = event,
           payload.phase == .running,
           let currentSession = state.session(id: payload.sessionID),
           currentSession.phase == .completed,
           payload.timestamp < currentSession.updatedAt {
            return
        }
    ...
    private func notificationSurfaceIsEligibleForPresentation(
        ...
        // Allow upgrading an already-open session list (click/hover/boot) into a
        // completion/permission notification card. Previously only
        // closed | notification was eligible, so Grok finish while the list was
        // open never triggered "刘海掉下来".
        let notchAllowsNotificationCard =
            notchStatus == .closed
            || notchOpenReason == .notification
            || notchOpenReason == .click
            || notchOpenReason == .hover
            || notchOpenReason == .boot

        return (ingress == .bridge || !isResolvingInitialLiveSessions)
            && notchAllowsNotificationCard
    ...
    /// Make a Grok session visible on the island without pretending it is a
    /// permission/completion notification card (those only accept non-running).
    private func surfaceGrokSessionVisibility(sessionID: String, reason: String) {
        synchronizeSelection()
        refreshOverlayPlacementIfVisible()
