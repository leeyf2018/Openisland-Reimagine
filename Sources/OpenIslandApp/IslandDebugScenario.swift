...
enum IslandDebugScenario: String, CaseIterable, Identifiable {
    ...
    var summary: String {
        ...
        case .grokCompletionCard:
            "Auto-expanded Grok completion surface uses the numbered digest instead of the raw reply."
        case .longCompletionCard:
    ...
    func snapshot(at now: Date = .now) -> IslandDebugSnapshot {
        ...
        case .sessionList:
            ...
            return IslandDebugSnapshot(
                ...
                codexUsageSnapshot: CodexUsageSnapshot(
                    sourceFilePath: "debug://codex-usage",
                    capturedAt: now,
                    windows: [
                        CodexUsageWindow(
                            key: "secondary",
                ...
                grokUsageSnapshot: GrokUsageSnapshot(
                    sourceFilePath: "debug://grok-usage",
                    capturedAt: now,
                    usedPercentage: 42,
        ...
        case .grokCompletionCard:
            let session = DebugSessionFactory.grokCompletionSession(now: now)
            return IslandDebugSnapshot(
                title: title,
                summary: summary,
                previewHeight: 370,
...
private enum DebugSessionFactory {
    ...
    static func grokCompletionSession(now: Date) -> AgentSession {
        let digest = """
        1. 多 Grok 孤儿会话已通过唯一 session UUID 安全恢复，不会只凭 PID 生成无法对应真实任务的幽灵条目
        2. 同一 PID 的历史 registry 记录会先归一，只保留最近打开的真实会话，避免旧条目和当前任务重复出现
        3. 自动完成摘要保持编号分行展示，不再把所有内容压缩成一条长文本，阅读时可以快速区分每项结果
        4. 通知专用窗口小幅加宽，让较长的中文摘要减少不必要的自动换行，同时不改变普通手动列表的尺寸
        5. 摘要可视区域适度增高，默认可以直接看到约四到五条结果，减少连续多次双指滚动才能读完的问题
        6. 超出可视区域的后续内容仍然保留滚动能力，不会被截断或丢失，也不会让窗口无限向下占满办公画面
        7. 正式 App 和 Codex、Cursor hooks 在源码验收阶段均保持不变，只有获得确认后才会晋升新的正式版本
        """
        return AgentSession(
            id: "019fc5e3-a1de-7792-9384-913e2045c491",
            title: "Grok · OpenIsland",
            tool: .grok,
            origin: .demo,
            attachmentState: .attached,
            phase: .completed,
            ...
            jumpTarget: JumpTarget(
                ...
                paneTitle: "grok ~/open-island",
                workingDirectory: "/tmp/open-island",
                terminalSessionID: "ghostty-grok-completion"
            ),
            grokMetadata: GrokSessionMetadata(
                sessionTitle: "OpenIsland",
                initialUserPrompt: "修复多 Grok 状态和自动摘要展示。",
                lastUserPrompt: "把自动完成摘要窗口稍微放大，默认能看到四五点。",
                lastAssistantMessage: "这是故意很长的原始助手回复；正确实现不应优先显示这一整段，而应展示七条编号摘要并保留适度滚动。",
