...
final class OverlayPanelController {
    ...
    private func panelSize(for model: AppModel?, on screen: NSScreen) -> CGSize {
        ...
        let isNotificationMode = model.notchOpenReason == .notification
            && model.islandSurface.sessionID != nil
        let panelWidth = isNotificationMode
    ...
    private func completionBodyHeight(for session: AgentSession, model: AppModel) -> CGFloat {
        ...
        let text = (session.completionNotificationMessageText ?? session.summary)
        ...
        let markdownHeight = min(200, ceil(textSize.height) + 20)
