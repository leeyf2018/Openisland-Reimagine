...
struct IslandPanelView: View {
    ...
    /// Pull chips right so the leading digit of C (e.g. “1” in 100) is not clipped.
    private static let notchHeaderLeadingPadding: CGFloat = 22
    private static let notchHeaderTrailingPadding: CGFloat = 46
    private static let notchLaneSafetyInset: CGFloat = 10
    /// Gap between C / G / O chips.
    private static let notchUsageChipSpacing: CGFloat = 3
    /// Inner padding of each chip frame.
    private static let notchUsageChipHorizontalPadding: CGFloat = 3
    private static let minimumRightUsageLaneWidth: CGFloat = 44
    /// Min header height so C/G/O chips (letter + number + days) fit.
    private static let usageHeaderMinHeight: CGFloat = 44
    /// One step smaller than 1.19 (letter / metric / reset days).
    private static let usageLetterFontSize: CGFloat = 12
    private static let usageMetricFontSize: CGFloat = 13
    private static let usageResetFontSize: CGFloat = 11
    /// Extra space between usage header and SESSIONS / total / running row.
    private static let sessionPanelHeaderTopPadding: CGFloat = 10
    ...
    private var openedHeaderLeadingPadding: CGFloat {
        usesNotchAwareOpenedHeader ? Self.notchHeaderLeadingPadding : Self.headerHorizontalPadding
    }

    private var openedHeaderTrailingPadding: CGFloat {
        usesNotchAwareOpenedHeader ? Self.notchHeaderTrailingPadding : Self.headerHorizontalPadding
    }

    private var usageChipSpacing: CGFloat {
        usesNotchAwareOpenedHeader ? Self.notchUsageChipSpacing : 7
    }

    private var usageChipHorizontalPadding: CGFloat {
        usesNotchAwareOpenedHeader ? Self.notchUsageChipHorizontalPadding : 8
    }
    ...
    private func openedSurface(width openedWidth: CGFloat, height openedHeight: CGFloat) -> some View {
        ...
        ZStack(alignment: .top) {
            ...
            VStack(spacing: 0) {
                // Usage chips are taller (letter + metric + reset days); give the
                // header a bit more room so enlarged type is not clipped.
                let headerHeight = max(closedNotchHeight, Self.usageHeaderMinHeight)
                openedHeaderContent
                    .frame(height: headerHeight)

                openedContent
                    .frame(width: openedWidth)
                    .frame(maxHeight: max(0, openedHeight - headerHeight), alignment: .top)
                    .clipped()
            }
    ...
    private var openedHeaderContent: some View {
        if usesNotchAwareOpenedHeader {
            GeometryReader { geometry in
                ...
                .padding(.leading, openedHeaderLeadingPadding)
    ...
    private func sessionPanelHeader(referenceDate: Date) -> some View {
        ...
        // Push SESSIONS / total / running down away from the usage chips.
        .padding(.top, Self.sessionPanelHeaderTopPadding)
        .frame(height: 36 + Self.sessionPanelHeaderTopPadding, alignment: .bottom)
    ...
    private var openedUsageSummary: some View {
        ...
        if providers.isEmpty == false {
            // No "%", no "7d" — number + optional (resetDays) under it.
    ...
    private var openedUsageProviders: [UsageProviderPresentation] {
        ...
        if model.showCodexUsage,
           let snapshot = model.grokUsageSnapshot {
            providers.append(
                UsageProviderPresentation(
                    id: "grok",
                    title: "G",
    ...
    private func usageLaneView(
        ...
        if providers.isEmpty {
            ...
            // Single-letter chips: `C 100` with optional `(4)` reset-days under the number.
            compactUsageSummaryView(providers, usesShortTitles: true)
    ...
    private func openedHeaderMetrics(for totalWidth: CGFloat) -> OpenedHeaderMetrics {
        let leadingPadding = openedHeaderLeadingPadding
        let trailingPadding = openedHeaderTrailingPadding
    ...
    private func compactUsageSummaryView(
        ...
        HStack(spacing: usageChipSpacing) {
        ...
        // Do not use fixedSize here: a wider HStack inside a narrower left
        // lane was clipping the leading letter (C disappeared, 100 remained).
        .frame(maxWidth: .infinity, alignment: .leading)
    }
    ...
    private func compactUsageChip(
        ...
        return VStack(spacing: 1) {
            Text(letter)
                .font(.system(size: Self.usageLetterFontSize, weight: .bold))
                .foregroundStyle(.white.opacity(0.95))
                .lineLimit(1)

            Text(metricText)
                .font(.system(size: Self.usageMetricFontSize, weight: .bold, design: .monospaced))
        ...
        .padding(.horizontal, usageChipHorizontalPadding)
        .padding(.vertical, 2)
    ...
    private func usageHelpText(for provider: UsageProviderPresentation) -> String {
        ...
        let windows = provider.windows.map { window in
            var parts = ["\(window.roundedUsedPercentage)% used"]
            if let days = window.resetDaysRemaining {
                parts.append("resets in \(days)d")
...
private struct UsageProviderPresentation: Identifiable {
    ...
    init(
        id: String,
        title: String,
        primaryDisplay: UsagePrimaryDisplay? = nil,
        windows: [UsageWindowPresentation]
    ) {
        self.id = id
        self.title = title
        // Default: show peak window % (Codex / Grok / Claude).
        if let primaryDisplay {
            self.primaryDisplay = primaryDisplay
        } else {
            let peak = windows.max { $0.usedPercentage < $1.usedPercentage }
            self.primaryDisplay = .percentUsed(peak?.roundedUsedPercentage ?? 0)
        }
    ...
    /// For color thresholds (0–100+). Still based on plan/window utilization.
    ...
    /// Text shown under the letter: credits for O, percent for C/G.
    var primaryMetricText: String {
        switch primaryDisplay {
        case .percentUsed(let value):
            "\(value)"
...
private struct UsageWindowPresentation: Identifiable {
    ...
    /// Ceiling of days until reset. Past/missing → nil (hide `(n)`).
...
private struct IslandSessionRow: View {
    ...
    private func rowAuxiliaryDetails(presence: IslandSessionPresence) -> some View {
        if !shouldShowEmbeddedDetailBody,
           let activityLine = session.spotlightActivityLineText ?? expandedActivityLineText {
            // Notification completion cards need more vertical room so the
            // distilled 1–5 bullet digest is readable (Codex used to show only
            // the first ~2 lines of the raw reply).
            // Show up to 3 digest lines for completed rows (list + popup).
            let isCompletedDigest = session.phase == .completed
            Text(activityLine)
                .font(.system(size: 11, weight: .medium))
                .foregroundStyle(activityColor(for: presence).opacity(0.94))
                .lineLimit(isCompletedDigest ? 4 : 2)
                .multilineTextAlignment(.leading)
                .fixedSize(horizontal: false, vertical: isCompletedDigest)
                .padding(.leading, detailLeadingInset)
                .padding(.trailing, sideInset)
                .padding(.bottom, 10)
        }
    ...
    private var completionActionBody: some View {
        VStack(alignment: .leading, spacing: 0) {
            if !completionMessageText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                AutoHeightScrollView(maxHeight: 200) {
    ...
    private var completionMessageText: String {
        if let text = session.completionNotificationMessageText?.trimmedForNotificationCard, !text.isEmpty {
            return text
        }
    ...
    private func statusIndicator(for presence: IslandSessionPresence) -> some View {
        ...
        return Group {
            ...
            case .animatedDot:
                animatedStatusIndicator(tint: tint, presence: presence)
            case .bar:
        ...
        .accessibilityLabel(Text(statusAccessibilityLabel(for: presence)))
    }

    @ViewBuilder
    private func animatedStatusIndicator(tint: Color, presence: IslandSessionPresence) -> some View {
        switch IslandAnimatedStatusVisual.resolve(phase: session.phase, presence: presence) {
        case .runningWaveform:
            if !accessibilityReduceMotion,
               let interval = stateIndicator.timelineInterval(
                   presence: presence,
                ...
                TimelineView(.periodic(from: .now, by: interval)) { context in
                    runningWaveform(
                        tint: tint,
                        phase: context.date.timeIntervalSinceReferenceDate * 7
                    )
                }
            } else {
                runningWaveform(tint: tint, phase: 0)
            }
        case .approvalAttention:
            attentionStatusGlyph(
                systemName: "exclamationmark.triangle.fill",
                tint: tint,
                presence: presence
            )
        case .answerAttention:
            attentionStatusGlyph(
    ...
    private func runningWaveform(tint: Color, phase: TimeInterval) -> some View {
        HStack(alignment: .center, spacing: IslandRunningWaveformMetrics.barSpacing) {
            ForEach(0..<IslandRunningWaveformMetrics.barCount, id: \.self) { index in
                RoundedRectangle(cornerRadius: 1, style: .continuous)
                    .fill(tint)
                    .frame(
                        width: IslandRunningWaveformMetrics.barWidth,
                        height: IslandRunningWaveformMetrics.barHeight(index: index, phase: phase)
                    )
            }
        }
        .frame(width: 14, height: 12, alignment: .center)
        .shadow(color: tint.opacity(0.32), radius: 2)
        .frame(width: 14, height: 24, alignment: .center)
    }

    @ViewBuilder
    private func attentionStatusGlyph(
        systemName: String,
        tint: Color,
        ...
        if !accessibilityReduceMotion,
           let interval = stateIndicator.timelineInterval(
            ...
            TimelineView(.periodic(from: .now, by: interval)) { context in
                ...
                attentionGlyph(systemName: systemName, tint: tint, pulse: pulse)
            }
        } else {
            attentionGlyph(systemName: systemName, tint: tint, pulse: 0)
        }
    }

    private func attentionGlyph(systemName: String, tint: Color, pulse: Double) -> some View {
        Image(systemName: systemName)
            .font(.system(size: 10, weight: .bold))
            .foregroundStyle(tint)
            .scaleEffect(1 + (pulse * 0.1))
            .shadow(color: tint.opacity(0.3 + (pulse * 0.24)), radius: 2 + (pulse * 2))
            .frame(width: 14, height: 24, alignment: .center)
    }

    private func statusAccessibilityLabel(for presence: IslandSessionPresence) -> String {
        switch IslandAnimatedStatusVisual.resolve(phase: session.phase, presence: presence) {
        case .runningWaveform:
            return lang.t("island.section.inProgress")
        case .approvalAttention:
            return lang.t("island.section.needsApproval")
        case .answerAttention:
            return lang.t("island.section.needsAnswer")
        case .completedBadge:
            return lang.t("island.section.justDone")
        case .idleDot:
            return lang.t("island.section.idle")
        }
    }
