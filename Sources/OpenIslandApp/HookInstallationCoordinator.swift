...
final class HookInstallationCoordinator {
    ...
    /// periodic timer (which used to be 120s and lagged the website).
    @ObservationIgnored
    private var grokUsageLogWatchSource: DispatchSourceFileSystemObject?

    @ObservationIgnored
    private var grokUsageLogWatchFileDescriptor: Int32 = -1
    ...
    func refreshGrokUsageState() {
        Task { [weak self] in
            ...
            do {
                let snapshot = try await Task.detached(priority: .utility) {
                    try GrokUsageLoader.load()
                }.value
                // Always assign so Observation notifies even when only the
                // percentage field changed inside an otherwise similar snapshot.
                if self.grokUsageSnapshot != snapshot {
                    self.grokUsageSnapshot = snapshot
                }
            } catch {
                self.onStatusMessage?("Failed to read Grok usage state: \(error.localizedDescription)")
            }
        }
    }
    ...
    func refreshOpenCodeUsageState() {
        Task { [weak self] in
            ...
            do {
                ...
                if self.openCodeUsageSnapshot != snapshot {
                    self.openCodeUsageSnapshot = snapshot
                }
            } catch {
                // Keep last snapshot; avoid spamming the status bar every 15s.
            }
        }
    }
    ...
    func startGrokUsageMonitoringIfNeeded() {
        if grokUsageMonitorTask == nil {
            grokUsageMonitorTask = Task { @MainActor [weak self] in
                guard let self else { return }

                while !Task.isCancelled {
                    self.refreshGrokUsageState()
                    // Website usage moves during a turn; the local log only
                    // updates when Grok CLI refetches billing. Re-read often
                    // enough that a new log line shows on the island within
                    // ~15s instead of waiting two minutes.
    ...
    func startOpenCodeUsageMonitoringIfNeeded() {
        ...
        openCodeUsageMonitorTask = Task { @MainActor [weak self] in
            guard let self else { return }
    ...
    /// Near-real-time refresh when `unified.jsonl` is appended (session start,
    /// mid-session billing refresh). Complements the 15s timer.
    private func startGrokUsageLogWatcherIfNeeded() {
        guard grokUsageLogWatchSource == nil else { return }

        let path = GrokUsageLoader.defaultLogURL.path
        guard FileManager.default.fileExists(atPath: path) else { return }

        let fd = open(path, O_EVTONLY)
        guard fd >= 0 else { return }

        grokUsageLogWatchFileDescriptor = fd
        ...
        source.setEventHandler { [weak self] in
            ...
            // Log rotation / replace: drop the old fd watch and re-arm.
            let data = source.data
            if data.contains(.rename) || data.contains(.delete) {
                self.stopGrokUsageLogWatcher()
                self.startGrokUsageLogWatcherIfNeeded()
            }
        }
        ...
        grokUsageLogWatchSource = source
        source.resume()
    }

    private func stopGrokUsageLogWatcher() {
        grokUsageLogWatchSource?.cancel()
        grokUsageLogWatchSource = nil
        grokUsageLogWatchFileDescriptor = -1
    }
