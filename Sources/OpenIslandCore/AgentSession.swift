...
public enum AgentTool: String, CaseIterable, Codable, Sendable {
    ...
    case cursor
    case kimiCLI
    /// Grok Build TUI (local discovery via `~/.grok/active_sessions.json` + process scan).
    /// Not hook-managed — never written by Setup installers.
    case grok
...
public extension AgentSession {
    ...
    var isTrackedLiveSession: Bool {
        !isDemoSession && (tool == .codex || tool == .claudeCode || tool == .geminiCLI || tool == .openCode || tool == .qoder || tool == .qwenCode || tool == .factory || tool == .codebuddy || tool == .cursor || tool == .kimiCLI || tool == .grok)
    }
    ...
    var lastAssistantMessageText: String? {
        if let value = codexMetadata?.lastAssistantMessage { return value }
        if let value = claudeMetadata?.lastAssistantMessage { return value }
        if let value = geminiMetadata?.lastAssistantMessage { return value }
        if let value = openCodeMetadata?.lastAssistantMessage { return value }
        if let value = cursorMetadata?.lastAssistantMessage { return value }
        if let value = grokMetadata?.lastAssistantMessage { return value }
        return nil
    }
    ...
    /// Text shown in the automatic completion card. Grok already builds a
    /// concise numbered digest for the session row, so reuse it here instead of
    /// expanding the raw assistant reply into one visually crowded block.
    var completionNotificationMessageText: String? {
        if let grokDigest = grokMetadata?.completionDigest?
            .trimmingCharacters(in: .whitespacesAndNewlines),
           !grokDigest.isEmpty {
            return grokDigest
        }
        return completionAssistantMessageText
    }
    ...
    var latestUserPromptText: String? {
        if let value = codexMetadata?.lastUserPrompt { return value }
        if let value = claudeMetadata?.lastUserPrompt { return value }
        if let value = geminiMetadata?.lastUserPrompt { return value }
        if let value = openCodeMetadata?.lastUserPrompt { return value }
        if let value = cursorMetadata?.lastUserPrompt { return value }
        if let value = grokMetadata?.lastUserPrompt { return value }
        return nil
    }

    var initialUserPromptText: String? {
        if let value = codexMetadata?.initialUserPrompt { return value }
        if let value = claudeMetadata?.initialUserPrompt { return value }
        if let value = geminiMetadata?.initialUserPrompt { return value }
        if let value = openCodeMetadata?.initialUserPrompt { return value }
        if let value = cursorMetadata?.initialUserPrompt { return value }
        if let value = grokMetadata?.initialUserPrompt { return value }
        if let value = grokMetadata?.sessionTitle { return value }
        return nil
    }
