...
struct AppModelSessionListTests {
    ...
    @Test
    func staleRolloutRunningEventDoesNotReopenCompletedSession() {
        let now = Date(timeIntervalSince1970: 2_000)
        let model = AppModel()
        model.state = SessionState(
            sessions: [
        ...
        model.applyTrackedEvent(
            .activityUpdated(
                SessionActivityUpdated(
                    ...
                    summary: "Stale rollout activity.",
                    phase: .running,
                    timestamp: now.addingTimeInterval(-1)
                )
            ),
        ...
        #expect(model.liveRunningCount == 0)
    }

    @Test
    func currentRolloutRunningEventReopensCompletedSessionForResumedTurn() {
        let now = Date(timeIntervalSince1970: 2_000)
        let model = AppModel()
        model.state = SessionState(
            sessions: [
        ...
        model.applyTrackedEvent(
            .activityUpdated(
                SessionActivityUpdated(
                    ...
                    summary: "Prompt: Resume the task.",
                    phase: .running,
                    timestamp: now
                )
            ),
        ...
        #expect(model.state.session(id: "resumed-session")?.phase == .running)
        #expect(model.liveRunningCount == 1)
    }
