...
struct AgentSessionPresentationTests {
    ...
    @Test
    func grokCompletionNotificationPrefersNumberedDigestOverRawReply() {
        let digest = "1. 修复多 Grok 发现\n2. 自动摘要保持分行\n3. 测试通过"
        let session = AgentSession(
            ...
            grokMetadata: GrokSessionMetadata(
                lastAssistantMessage: "这是完整回复，自动完成卡片不应优先显示这一大段内容。",
                completionDigest: digest
            )
        )

        #expect(session.completionNotificationMessageText == digest)
        #expect(session.completionNotificationMessageText?.components(separatedBy: "\n").count == 3)
    }
}
